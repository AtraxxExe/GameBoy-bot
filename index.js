...
client.once(Events.ClientReady, async () => {
  ...
  try {
    ...
    if (needsCatchUp && Object.keys(guildConfigs).length > 0) {
      console.log("[FREE GAMES] Catch-up announcement triggered (bot was offline at 00:00 UTC)");
      await announceDailyFreeGames(client);
    }
  } catch (err) {
    console.error("❌ Failed to initialize free games system:", err);
  }
