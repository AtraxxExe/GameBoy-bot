...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === "user") {
      ...
      audit("ban", { action: "Ban command used", actor: interaction.user, target: user, reason });
      await notifyAdmin(interaction.guild, { embeds: [new EmbedBuilder().setColor(0xd32f2f).setTitle("Member banned").setDescription(`Admin: <@${interaction.user.id}>\nUser: <@${user.id}>\nReason: ${reason}`).setTimestamp()] });
