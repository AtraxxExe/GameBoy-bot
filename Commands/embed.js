...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await interaction.reply({ 
      content: "🛠️ I’ve started the embed setup. Follow the prompts and type `!stop!` at any time to cancel.",
    ...
    try {
      if (isRules) {
        const chanPrompt = await channel.send("👉 Step 1: Tell me where you want the embed posted.");
      ...
      if (!embedHeader && !embedBody && !embedImage) {
        ...
        return channel.send("❌ The embed needs some content before I can post it.").then(m => setTimeout(() => m.delete().catch(() => {}), 5000));
      }
      ...
      if (imgUserMsg) await imgUserMsg.delete().catch(() => {});
      await channel.send(`✅ The embed is now live in ${targetPostChannel}.`).then(m => setTimeout(() => m.delete().catch(() => {}), 4000));
