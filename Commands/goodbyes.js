...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === "remove") {
      if (!existing) {
        return interaction.reply({ content: "There isn’t a goodbye setup to change right now.", flags: [MessageFlags.Ephemeral] });
      }
    ...
    if (sub === "test") {
      if (!existing) {
        return interaction.reply({ content: "There isn’t a goodbye setup to test yet. Run `/goodbyes set` first.", flags: [MessageFlags.Ephemeral] });
      }
      ...
      return interaction.reply({ content: `✅ I sent a test goodbye message to <#${goodbyeChannel.id}>.`, flags: [MessageFlags.Ephemeral] });
    }
    ...
    if (sub === "modify") {
      if (!existing) {
        return interaction.reply({ content: "There isn’t a goodbye setup to change right now. Run `/goodbyes set` first.", flags: [MessageFlags.Ephemeral] });
      }
      ...
      return interaction.reply({ content: "A goodbye setup already exists. Use `/goodbyes modify` or `/goodbyes remove`.", flags: [MessageFlags.Ephemeral] });
    }

    await interaction.reply({ content: `🛠️ I’ve started the goodbye setup. Type \`!stop!\` anytime to cancel.`, flags: [MessageFlags.Ephemeral] });
    ...
    try {
      ...
      if (runChannel) {
        ...
        if (!targetChannel) {
          ...
          if (!targetChannel) return channel.send("❌ Setup stopped.");
        }
      ...
      return channel.send(`🎉 Goodbye messages are ready to go in <#${targetChannelId}>.`).then(m => setTimeout(() => m.delete().catch(() => {}), 5000));

    } catch (err) {
      console.error(err);
      return channel.send("❌ The setup stopped because something went wrong.").then(m => setTimeout(() => m.delete().catch(() => {}), 5000));
    }
  }
};