...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await interaction.reply({ 
      content: "🛠️ **Auto Roles Wizard Initiated.** Follow the prompts below to build your interaction role cards.\nYou can type `!stop!` at any step to cancel the setup process.", 
    ...
    const checkStop = async (collectedMessage, promptMessage) => {
      if (collectedMessage.content.trim() === "!stop!") {
        ...
        await channel.send("🛑 **Wizard Cancelled.** The auto role setup has been terminated.").then(m => setTimeout(() => m.delete().catch(() => {}), 5000));
        return true;
      }
      return false;
    };
    ...
    try {
      const chanPrompt = await channel.send("👉 **Step 1:** Mention the channel where the reaction roles embed card should be posted (e.g., #roles).");
      ...
      if (mentionedChan) {
        ...
        const retryChanPrompt = await channel.send("⚠️ **Invalid channel.** Please try one more time. Mention a valid text channel (e.g., #roles) or type `!stop!` to cancel.");
      ...
      if (/^[0-9A-F]{6}$/i.test(rawColor)) {
        ...
        const retryMsg = await channel.send("⚠️ **Invalid Hex format.** Please try one more time. Send a valid hex code like `#FF0000` or type `!stop!` to cancel.");
      ...
      const loopInstructionMsg = await channel.send(
        "✨ **Step 4:** Send your emoji, role name, and optional hex color one message at a time.\n" +
      ...
      while (collectingRoles) {
        ...
        if (rawContent.toLowerCase() === "!stop!") {
          ...
          await channel.send("🛑 **Wizard Cancelled.** The workflow process has been stopped.").then(m => setTimeout(() => m.delete().catch(() => {}), 5000));
          return;
        }
        ...
        if (!matchPattern) {
          const warnMsg = await channel.send("⚠️ **Invalid formatting entry layout.** Make sure it looks like: `:emoji: Role Name` or `:emoji: Role Name #hex`.");
          await entryUserMsg.delete().catch(() => {});
        ...
        if (!guildRole) {
          try {
            ...
            const warnMsg = await channel.send("❌ **Failed to create role.** Check that GameBoy has permissions to manage server roles.");
            await entryUserMsg.delete().catch(() => {});
            setTimeout(() => warnMsg.delete().catch(() => {}), 5000);
            continue;
          }
      ...
      await channel.send(`🎉 **Thank you. Embed created successfully.** Role card shipped to ${targetPostChannel}!`).then(m => setTimeout(() => m.delete().catch(() => {}), 5000));

    } catch (err) {
      console.error(err);
      await channel.send("❌ Wizard expired due to inactivity.").then(m => setTimeout(() => m.delete().catch(() => {}), 5000));
    }
  }
};