...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === "set") {
      ...
      return interaction.reply({ content: "✅ Level up announcements setup successfully.", flags: [MessageFlags.Ephemeral] });
    }
    ...
    if (sub === "reset") {
      ...
      if (typeof levelManager.resetGuild === 'function') {
        ...
        try {
          ...
          return interaction.editReply({ content: "⚠️ Levels were reset, but an error occurred while removing milestone roles from members. Please check my role hierarchy permissions!" });
        }
      }

      return interaction.editReply({ content: "⚠️ Reset function is not available." });
    }
  }
};